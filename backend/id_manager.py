...
# ── Config ─────────────────────────────────────────────────────────────────
STABILITY_WINDOW = 15  # Reduced for faster ID assignment
STABILITY_THRESHOLD = 8  # Reduced (was 18/25)
MAX_PER_TEAM = 7
...
def _cosine_sim(emb1: np.ndarray, emb2: np.ndarray) -> float:
    return float(
        np.dot(emb1, emb2) / (np.linalg.norm(emb1) * np.linalg.norm(emb2) + 1e-6)
    )


def _get_mean_emb(tid: int) -> Optional[np.ndarray]:
    if tid not in _emb_history or not _emb_history[tid]:
        return None
...
def _update_track_state(tid, box, emb):
    if tid in _last_box:
        ...
        pc = (
            (_last_box[tid][0] + _last_box[tid][2]) / 2,
            (_last_box[tid][1] + _last_box[tid][3]) / 2,
        )
...
def try_assign_ids() -> Dict[int, str]:
    ...
    for tid, pwin in _presence.items():
        if tid in _locked:
            continue
        if len(pwin) < STABILITY_WINDOW or sum(pwin) < STABILITY_THRESHOLD:
            continue
        if tid not in _team_snapshot:
            continue

        team = _team_snapshot[tid]
        if _team_counters[team] > MAX_PER_TEAM:
            continue
...
def cleanup_stale_ids():
    ...
    to_prune = [
        tid
        for tid, last_f in _last_frame.items()
    ...
    for tid in to_prune:
        for d in [
            ...
            if tid in d:
                del d[tid]
    if to_prune:
        print(f"[GC] Pruned {len(to_prune)} transient tracks.")
