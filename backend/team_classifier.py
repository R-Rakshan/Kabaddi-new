...
# ── Config ─────────────────────────────────────────────────────────────────
OBS_WINDOW       = 60    # frames to keep per track (larger = more stable)
MIN_OBS          = 15    # minimum observations before classifying
...
def _extract_torso_hsv(frame: np.ndarray, x1: int, y1: int, x2: int, y2: int) -> Optional[np.ndarray]:
    """
...
def update_observations(frame: np.ndarray, track_boxes: Dict[int, Tuple], frame_idx: int = 0) -> None:
...
def _build_or_update_anchors(means_x: Dict[int, float], means_hsv: Dict[int, np.ndarray]) -> None:
    ...
    _, labels_c, centers_hsv = cv2.kmeans(hsv_arr, 2, None, crit, 10, cv2.KMEANS_RANDOM_CENTERS)
...
def _classify_single(tid: int, mean_x: float, mean_hsv: Optional[np.ndarray]) -> Optional[int]:
